...
class manage{
	...
	private function services($patient, $date ){
		$services = "";
		$result = $this->db->getAll("SELECT diagnose from diagnosis where date = ? and patient_id = ? and payment = 0 and lab = 1 and services_id is null",array($date, $patient));
	...
	public function getData($query){
		...
		$data[] = "<div class='row'>
					...
					<a href='lab.php?pid=".$query['patient_id']."&date=".$query['date']."' class='btn btn-primary btn-block'>Test</a></div>
...
$sql.=" FROM diagnosis d INNER JOIN patient p ON p.id = d.patient_id WHERE lab = 1 and (payment = 0 or p.ward is not null) and (services_id is null)";
