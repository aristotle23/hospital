...
class manage{
	...
	private function services($patient, $date ){
		$services = "";
		$result = $this->db->getAll("SELECT diagnose from diagnosis where lab_date = ? and patient_id = ? and payment = 0 and lab = 2 and services_id is null",array($date, $patient));
		foreach($result as $key => $service){
			if($key == 0){
				$services .= $service['diagnose'];
				continue;
			}
			$services .= ", ".$service['diagnose'];
		}
	...
	public function getData($query){
		...
		$data[] = "<div class='row'>
					...
					<a href='edit_lab.php?pid=".$query['patient_id']."&date=".$query['date']."' class='btn btn-primary btn-block'>Edit</a></div>
...
$sql = "SELECT distinct(d.lab_date) as date, patient_id ";
$sql.=" FROM diagnosis d INNER JOIN patient p ON p.id = d.patient_id WHERE lab = 2 and (payment = 0 or p.ward is not null) and (services_id is null)";
